...
class GameViewController: UIViewController, SCNSceneRendererDelegate, SCNPhysicsContactDelegate {
    ...
    override func viewDidLoad() {
        ...
        if self.ship == nil {
            // Fallback si el modelo raiz no tiene nombre "ship"
            self.ship = scene.rootNode.childNodes.first
            self.ship?.name = "ship"
        }

        // Configuracion fisica de la nave
        if let ship = self.ship {
            let (minVec, maxVec) = ship.boundingBox
            let dx = CGFloat(maxVec.x - minVec.x)
            let dy = CGFloat(maxVec.y - minVec.y)
            let dz = CGFloat(maxVec.z - minVec.z)

            ship.physicsBody = SCNPhysicsBody(
                type: .kinematic,
                shape: SCNPhysicsShape(geometry: SCNBox(
                    width: max(dx, 1.0),
                    height: max(dy, 1.0),
                    length: max(dz, 1.0),
                    chamferRadius: 0
    ...
    func setupTitleAndGameOver() {
        ...
        titleNode.position = SCNVector3(-30, 15, -30)
        ...
        tapNode.position = SCNVector3(-18, 7, -20)
        ...
        gameOverNode.position = SCNVector3(-20, 12, -30)
        ...
        resultsNode.position = SCNVector3(-30, 5, -20)
    ...
    func setupLimits(forView view: SCNView) {
        // TODO [B06] Calcular y almacenar en `self.limits` el rectángulo que defina los límites de la zona "jugable"
        guard let pointOfView = view.pointOfView else {
            self.limits = CGRect(x: -35, y: -80, width: 70, height: 160)
            return
        }

        func pointOnPlane(screenPoint: CGPoint) -> SCNVector3? {
            let near = view.unprojectPoint(SCNVector3(Float(screenPoint.x), Float(screenPoint.y), 0.0))
            let far = view.unprojectPoint(SCNVector3(Float(screenPoint.x), Float(screenPoint.y), 1.0))
            ...
            if abs(dir.y) < 0.0001 { return nil }
        ...
        ].compactMap { pointOnPlane(screenPoint: $0) }

        if points.count == 4 {
            let minX = CGFloat(points.map { $0.x }.min() ?? -35)
            let maxX = CGFloat(points.map { $0.x }.max() ?? 35)
            let minZ = CGFloat(points.map { $0.z }.min() ?? -80)
            let maxZ = CGFloat(points.map { $0.z }.max() ?? 80)
            self.limits = CGRect(x: minX, y: minZ, width: maxX - minX, height: maxZ - minZ)
        } else {
            self.limits = CGRect(x: -35, y: -80, width: 70, height: 160)
        }
    ...
    func spawnAsteroid(pos: SCNVector3) {
        ...
        asteroid.physicsBody = SCNPhysicsBody(
            type: .kinematic,
            shape: SCNPhysicsShape(node: asteroid, options: nil)
        )
        ...
        let move = SCNAction.move(
            to: SCNVector3(pos.x, 0, Float(limits.maxY)),
            duration: 3.0
        )
    ...
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        ...
        // TODO [B10] Mueve la nave lateralmente
        let nextX = ship.position.x + velocity * 200 * Float(deltaTime)
    ...
    // TODO [C08] Definir el método `physicsWorld(:, didBegin:)`
